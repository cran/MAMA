...
%\addcontentsline{toc}{section}{Usage}
\subsection*{Data preparation}
Function {\ttfamily fold.change} calculates mean fold-change in MetaArray object. It has two arguments: data set (e.g. {\ttfamily ColonData}) and name of the column with class labels to be used (e.g. {\ttfamily "satelite"}). It assumes data are on $log_2$ scale.
<<VennFCH>>=
