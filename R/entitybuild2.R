...
entitybuild2 <- function(expr.mat,ALLtype=NULL,type,dataset=NULL,minSampleNum=3,method="t",random=FALSE)
  {
  n.patient <- length(ALLtype)
# method are "t" or "fc",or "z"
  cat("entities building")
# cat(ALLtype)

   if (ncol(expr.mat)!=length(ALLtype)) stop("expr.mat and ALLtype not match!")
  n.type <- length(type)
...
#           j<-3
	...
	twogroup <- c(groupA,groupB)
      ...
      lab <- c(rep(1,nA),rep(0,nB))
###########
## the higher label minus the lower label ########
###########
      thist <- mt.teststat(expr.mat[,twogroup],lab,test=method)
      t <- cbind(t,thist)
...
#   cat("resort the result gene list \n")

 if(!random)   return(list(entity=entity,stat=t))
 else return(stat=t)
}