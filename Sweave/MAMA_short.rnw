...
This paper provides a user guide to R-package MAMA. The package implements eight different methods that are designed to identify differentially expressed genes.\par
...
@
we combine study specific p-values({\ttfamily pval}) in {\ttfamily ColonData} dataset using column {\ttfamily "MSIstatus"} from clinical data as class labels. {\ttfamily pval} is same as {\ttfamily pvalt}.
%\section*{Discussion}
...
<<RandomScore>>=
random.score<-RandomScore(data = ColonData, varname = "MSIstatus", B = 10, alpha = A, test = "T", which=c("random", "empirical", "subsample"), two.sided = TRUE)
@
...
<<>>=
lists<-join.DEG(pvalt, ESt, ScoresFDR, SOGL.res, RankRes, z.stat, probs,genenames=rownames(GEDM(ColonData)[[1]]), type=c(1,1,3,4,5,6,7), cutoff=0.02)
#or
lists2<-join.DEG(pval, es, es2, SOGL.res, rp, z.stat,  map,genenames=rownames(GEDM(ColonData)[[1]]),  cutoff=0.05)
