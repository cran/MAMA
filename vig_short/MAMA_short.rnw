%\VignetteDepends{MAMA,methods, metaArray, tspair, xtable, metaMA, gtools, gplots, multtest, RankProd, MergeMaid, GeneMeta} 
...
This paper provides a user guide to R-package MAMA. The package implements nine different methods that are designed to identify differentially expressed genes.\par
...
There is also a wrapper function for each method that outperforms all necessary step as one-line command.\par
...
@
\subsection*{Detection differentially expressed genes}
<<DEGp>>=
...
@
we combine study specific p-values({\ttfamily pval}) in {\ttfamily ColonData} dataset using column {\ttfamily "satelite"} from clinical data as class labels. {\ttfamily pval} is same as {\ttfamily pvalt}.
%\section*{Discussion}
...
@ 
\end{center}
According to Q-Q plot the hypothesis seems to be valid and fixed-effect model (FEM) should be used. However, we are going to use random-effect model (REM) in this vignette, too.\par  
Functions {\ttfamily mu.tau2} and {\ttfamily var.tau2} estimate combined effect size ({\ttfamily mu.tau2}) and variance ({\ttfamily var.tau2}). Each effect size is a weighted average of the effects for the individual data sets divided by its standard error. The weights are the reciprocal of the estimated variances.  
...
In REM we have to account between-study variability ($\tau^2$). Function {\ttfamily tau2.DL} provides DerSimonian's and Laird's \cite{DerSimonian} estimates of $\tau^2$ from Cochran's $Q$. 
<<>>=
#REM
num.studies <- 3
...
<<RandomScore>>=
random.score<-RandomScore(data = ColonData, varname = "satelite", B = 10, alpha = A, test = "T", which=c("random", "empirical", "subsample"), two.sided = TRUE)
@
...
sig<-sigScore(ordering, A.opt$alpha, 1000)
...
<<SOGLperform>>=
SOGL.res <- performSOGL(ColonData, varname = "satelite", test = "FCH", B = 100, which=c("score", "empirical"), min.weight = 1e-05, two.sided = TRUE )
@ 
...
<<rankDEG>>=
RP.out<-RPadvance(rankdata$dat,rankdata$cl,rankdata$origin,num.perm=50,logged=TRUE,na.rm=FALSE,gene.names=rownames(GEDM(ColonData)[[1]]), plot=FALSE)
@
...
<<>>=
rp <- RankProduct(ColonData, "satelite", num.perm = 50, gene.names = rownames(GEDM(ColonData)[[1]]) )
@
...
\subsection*{Wrapper function}
Function {\ttfamily TSP} needs no data preparation and provides both calculation of top-scoring pair and its significance.
<<>>=
...
@
\subsection*{Detecting differentially expressed genes}
We only set threshold for T-statistics (95 \% quantile). If there is a lot of genes and datasets in the meta-analysis one should consider higher quantiles. 
<<MAPTcutoff>>=
stat <- c(stat.real)
...
<<>>=
map <- MAP.Matches(ColonData, "satelite", t.cutoff = "95.00%", nperm = 100, sig.col = "p.col.strong")
@
...
<<METRAdat>>=
metra<-meta.test(ColonData, varname = "satelite")
head(metra$test)
@
\subsection*{Detecting differentially expressed genes}
...
<<>>=
METRA<-c(rownames(MC)[MC[,1]<0.05 & MC[,4]<0.05], rownames(MC)[MC[,2]<0.05 & MC[,4]<0.05])
METRA[1:10]
@
\subsection*{Wrapper function}
...
<<>>=
lists<-join.DEG(pvalt, ESt, ScoresFDR, SOGL.res, RankRes, z.stat, tsp, probs,genenames=rownames(GEDM(ColonData)[[1]]), type=c(1,1,3,4,5,6,7,8), cutoff=0.02)
#or
lists2<-join.DEG(pval, es, es2, SOGL.res, rp, z.stat, tsp, map,genenames=rownames(GEDM(ColonData)[[1]]),  cutoff=0.05)
...
metaheat2(MAT, col=c("red3", "royalblue2"), legend.names=c("DEG","noDEG"), discrete=TRUE, trace="none", dendrogram="none")
@       
\end{center}
...
<<>>=
results<-join.results(pvalt, ESt, theScores, ScoresFDR$two.sided, SOGL.res, RankRes, z.stat, probs, MC, RQ, type=c(1,1,5,5,2,3,5,4,5,5), genenames=rownames(GEDM(ColonData)[[1]]))

#or 
results2<-join.results(pval, es, es2, SOGL.res, rp, z.stat, map, metra, genenames=rownames(GEDM(ColonData)[[1]]))
...
\begin{center}
<<fig=true>>=
plotgene(gene, type=c(0,0,1,4,5,6,7,10), datalabels=c("denmark", "australia", "japan", "combined"))
@
\end{center}
@
